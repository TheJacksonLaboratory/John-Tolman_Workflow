...
## Loading Porject
```{r loading project}
load("data/e_g_snpg_samqc_129Sj.v265d_john.RData")
gm <- get(load("data/gm_allqc_129Sj.v265d_john.RData"))
...
#save into_john.pdf
pdf(file = "output/Percent_missing_genotype_data_qc_129Sj.v265d_john.pdf", width = 20, height = 20)
...
save(percent_missing,
     file = "data/percent_missing_id_qc_129Sj.v265d_john.RData")
...
save(summary.cg,
     file = "data/summary.cg_qc_129Sj.v265d_john.RData")

pdf(file = "output/Proportion_matching_genotypes_before_removal_of_bad_samples_qc_129Sj.v265d_john.pdf", width = 20, height = 20) 
...
#load the intensities.fst_qc_john.RData
#load("data/intensities.fst_john.RData")
#X and Y channel
xn <- x[,ind_ids(gm)]
xn <- xn[snps$marker,]
xnm <- rownames(xn)
...
#load the intensities.fst_john.RData
#load("data/heh/intensities.fst_john.RData")
#X and Y channel
...
#load("data/m_john.RData")
#load("data/nxo_john.RData")

m <- maxmarg(pr)
nxo <- count_xo(m)
#save(pr, file = "data/pr_john.RData")
#str(pr)
save(m, file = "data/m_qc_129Sj.v265d_john.RData")
save(nxo, file = "data/nxo_qc_129Sj.v265d_john.RData")

e <- calc_errorlod(gm, pr)
#str(e)
e <- do.call("cbind", e)
save(e, file = "data/e_qc_129Sj.v265d_john.RData")
...
#save crossover into_john.pdf
pdf(file = "output/number_crossover_qc_129Sj.v265d_john.pdf")
...
load("data/e_qc_129Sj.v265d_john.RData")
...
iplot(seq_along(errors_ind), errors_ind, indID=lab,
      chartOpts=list(xlab="Mouse", ylab="Percent genotyping errors", ylim=c(0, 8),
                     axispos=list(xtitle=25, ytitle=50, xlabel=5, ylabel=5)))
save(errors_ind, file = "data/errors_ind_qc_129Sj.v265d_john.RData")
...
save(pmis_mar, file = "data/percent_missing_marker_qc_129Sj.v265d_john.RData")
...
pdf(file = "output/Percent_missing_genotype_data_per_marker_qc_129Sj.v265d_john.pdf")
par(mar=c(5.1,0.6,0.6, 0.6))
hist(pmis_mar, breaks=seq(0, 100, length=201),
     main="", yaxt="n", ylab="", xlab="Percent missing genotypes")
rug(pmis_mar)
...
pdf(file = "output/genotype_frequency_marker_qc_129Sj.v265d_john.pdf")
...
save(gf_mar, file = "data/genotype_freq_marker_qc_129Sj.v265d_john.RData")
...
#load("data/e_g_snpg_samqc_john.RData")
...
pdf(file = "output/genotype_error_marker_qc_129Sj.v265d_john.pdf")
grayplot(pmis_mar, errors_mar,
         xlab="Proportion missing", ylab="Proportion genotyping errors")
dev.off()

save(errors_mar, file = "data/genotype_errors_marker_qc_129Sj.v265d_john.RData")
...
#save(bad_markers, file = "data/bad_markers_all_qc_john.RData")

#bad_markers <- find_markerpos(gm, names(errors_mar[errors_mar > 5]))
#save(bad_markers, file = "data/bad_markers_qc_john.RData")
