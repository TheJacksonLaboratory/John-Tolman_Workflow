...
filepaths = '/Users/corneb/Documents/MyJax/CS/Projects/Simon.John.Tolman/haplo.reconst/2020-12-02/b6.v265d/john_samples_sorted_sex/'
#setwd(filepaths)
...
gm <- read_cross2(paste0(filepaths,"qtl2/gm_sorted.json"))
...
#save into pdf
pdf(file = "output/Percent_missing_genotype_data_b6.v265d_john.pdf", width = 20, height = 20)
...
save(percent_missing,file = "data/percent_missing_id_b6.v265d_john.RData")
...
#summary.cg$index <- 1:nrow(summary.cg)

#percent_missing <- n_missing(gm, summary="proportion")*100
percent_missing.df <- as.data.frame(percent_missing)
percent_missing.df$ind1 <- rownames(percent_missing.df)
#percent_missing.df$ind2 <- rownames(percent_missing.df)

summary.cg.ms.1 <- merge(summary.cg, percent_missing.df, by =c("ind1"), all.x=T)

percent_missing.df$ind2 <- rownames(percent_missing.df)
summary.cg.ms.2 <- merge(summary.cg.ms.1, percent_missing.df, by =c("ind2"), all.x=T)
#summary.cg.ms <- summary.cg.ms.2[order(summary.cg.ms.2$index),]

summary.cg.ms <- summary.cg.ms.2[c(2,9,12,1,10,13,3:6)]
names(summary.cg.ms) <- c("ind1","iid1","percent.missing_ind1","ind2","iid2","percent.missing_ind2","prop_match","n_mismatch","n_typed","n_match")

rownames(summary.cg.ms) <- NULL
...
save(summary.cg,file = "data/summary.cg_b6.v265d_john.RData")
...
pheno_file_nick <- dir(path = filepathp, pattern = "DO-Lmx1b QTL mapping phenotype initiation.csv", full.names = TRUE)
...
gm.pheno <- gm.pheno1[c("Correct.Mouse.ID","Unique.Sample.ID","Unique Sample ID","Sample ID...4","Sample ID...5","Geneseek.Sample.ID","Directory",
                     ...
                     "Run.Date","Include","cr","max_IOP","max_corneal_haze","Pupil_not_round")]
...
#load("data/pr.RData")
pr<-readRDS(paste0(filepaths,"Jax.Columbia_John__GigaMUGA_genoprobs_36state_sorted.rds"))
#load("data/m.RData")
#load("data/nxo.RData")
...
#str(pr)
save(m, file = "data/m_b6.v265d_john.RData")
save(nxo, file = "data/nxo_b6.v265d_john.RData")

e <- calc_errorlod(gm, pr)
#str(e)
e <- do.call("cbind", e)
save(e, file = "data/e_b6.v265d_john.RData")

#snpg <- predict_snpgeno(gm, m)
#snpg <- do.call("cbind", snpg)
#save(snpg, file = "data/snpg_b6.v265d_john.RData")
...
pdf(file = "output/number_crossover_b6.v265d_john.pdf")
...
#load("data/e_b6.v265d_john.RData")
...
iplot(seq_along(errors_ind), errors_ind, indID=lab,
      chartOpts=list(xlab="Mouse", ylab="Percent genotyping errors", ylim=c(0, 8),
                     axispos=list(xtitle=25, ytitle=50, xlabel=5, ylabel=5)))
save(errors_ind, file = "data/errors_ind_b6.v265d_john.RData")
...
#load("data/snpg_b6.v265d_john.RData")
#snpg <- gm$geno
...
#pdf(file = "output/Percent_genotype_errors_obs_vs_predicted_b6.v265d_john.pdf",width = 20, height = 20) 
#par(pty="s")
#err_direct <- rowMeans(snpgn != gobs, na.rm=TRUE)*100
#errors_ind_0 <- rowSums(e > 0)[rownames(gm$covar)]/n_typed(gm)*100
#par(mar=c(4.1,4.1,0.6, 0.6))
...
bad.sample <- qc_info[qc_info$generation ==1 | qc_info$Number_crossovers <= 200 | qc_info$Number_crossovers >=1000 | qc_info$remove.id.duplicated == TRUE,]

save(qc_info, bad.sample, file = "data/qc_info_bad_sample_b6.v265d_john.RData")
...
save(gm_samqc, file = "data/gm_samqc_b6.v265d_john.RData")
...
#save(e,g,snpg, file = "data/e_g_snpg_samqc_b6.v265d_john.RData")
save(e,g, file = "data/e_g_snpg_samqc_b6.v265d_john.RData")
