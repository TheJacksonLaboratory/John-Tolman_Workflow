...
samphen <- samphen1[c("Correct.Mouse.ID","Unique.Sample.ID","Unique Sample ID","Sample ID...4","Sample ID...5","Geneseek.Sample.ID","Directory",
...
#for(i in 2){
  ...
  finalsum <- cbind(totphensam[c("Original.Sample.ID","Geneseek.Sample.ID","Mouse.ID")],
                   ...
                   totphensam[c('Total_Genotyped',"Percent_Genotyped","MURGIGV01_SNP.Count","Sex.x","Sex.y","Sex","Inferred.Sex","DO Generation","DO.Mother.Generation","DO.Generation",
                                 "Directory","DOB","Run.Date","Lmx1b allele","Lmx1b.allele","Lmx1b Genotype","Lmx1b.Genotype","Parental.Strain.GigaMUGA.ID","Parental Strain GigaMUGA ID","cr","Include","max_IOP","max_corneal_haze","Pupil_not_round")])
...
summary <- summary[summary$Include,]
...
which(summary$Sex.y != summary$Inferred.Sex)
#14  48  75  77  78  94 119 130

which(summary[c(38:40)] == "--")
...
summary[c(1:3,21,23,25:26,29:30)] %>%
...
summary[c(1:3,31,33,34,38:40)] %>%
  kable(escape = F,align = c("ccccccccc"),linesep ="\\hline") %>%
  kable_styling(full_width = F) %>%
  kable_styling("striped", full_width = F) %>%
  column_spec(1:7, width = "3cm") %>%
  row_spec(8 ,color= "white",background = "green") %>%
  row_spec(69 ,color= "white",background = "green") %>%
  row_spec(70 ,color= "white",background = "green") %>%
  row_spec(130 ,color= "white",background = "green") %>%
  row_spec(149 ,color= "white",background = "green") %>%
  row_spec(150 ,color= "white",background = "green") %>%
  row_spec(164 ,color= "white",background = "green") %>%
  row_spec(347 ,color= "white",background = "green") %>%
  row_spec(349 ,color= "white",background = "green") %>%
  row_spec(356 ,color= "white",background = "green")
...
dis$no_IOP <- ifelse(summary$max_IOP == '--', 'XX', '')
dis$no_corneal_haze <- ifelse(summary$max_corneal_haze == '--', 'XX', '')
dis$no_Pupil_not_round <- ifelse(summary$Pupil_not_round == '--', 'XX', '')
dis$low_call.rate <- ifelse(summary$cr <.90, 'XX', '')
dis$different_sex <- ifelse(summary$Sex.y != summary$Inferred.Sex, 'XX', '')

dis <- data.frame(dis)
disind <- subset(dis, dis$different_sex =="XX" | dis$low_call.rate == 'XX' | dis$no_IOP == 'XX'| dis$no_corneal_haze == 'XX'| dis$no_Pupil_not_round == 'XX')
...
disind[] %>% 
   mutate(
     no_IOP = ifelse(no_IOP == 'XX',
                  cell_spec(no_IOP, color = 'green',background = 'green'),
                  ''),
     no_corneal_haze = ifelse(no_corneal_haze == 'XX',
                  cell_spec(no_corneal_haze, color = 'green',background = 'green'),
                  ''),
     no_Pupil_not_round = ifelse(no_Pupil_not_round == 'XX',
                  cell_spec(no_Pupil_not_round, color = 'green',background = 'green'),
                  ''),
     low_call.rate = ifelse(low_call.rate == 'XX',
                  cell_spec(low_call.rate, color = 'red',background = 'red'),
                  ''),
